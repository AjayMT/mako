...
int main(int argc, char *argv[])
{
  if (argc < 5) {
    fputs("Usage: font_compiler <ttf> <font_name> <height> <monospace_width>\n"
          "\t<ttf> is the TTF file name and <font_name> sets the name of the generated font.\n"
          "\tThe font is scaled to be <height> pixels tall.\n"
          "\tZero or negative <monospace_width> generates a variable-width font.\n"
          "\tPositive <monospace_width> generates a monospace font with the specified width.\n", stdout);
    return 1;
  }
  ...
  printf("#define %s_HEIGHT %u\n"
         "static uint8_t %s_data[] = {\n", font_name_upper, total_height, font_name_lower);
  ...
  for (char c = 32; c < 127; ++c) {
    ...
    if (l_bearing < 0) l_bearing = 0;
    if (l_bearing + w > advance_width) advance_width = l_bearing + w;
    ...
    for (int y = 0; y < padding_top; ++y) {
      for (int x = 0; x < advance_width; ++x) fputs(" 0,", stdout);
    ...
    for (int y = 0; y < h; ++y) {
      ...
      for (int x = monospace_xoff; x < w; ++x) printf(" %d,", bitmap[y * w + x]);
  ...
  for (char c = 32; c < 127; ++c)
    printf("  // '%c'\n  { .width = %u, .data_offset = %u },\n",
           c, infos[c - 32].width, infos[c - 32].data_offset);
