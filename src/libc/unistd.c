...
int32_t execve(const char *path, char *const argv[], char *const envp[])
{
  ...
  if (envp == NULL)
    envp = largv;
  int32_t res = _syscall3(SYSCALL_EXECVE, (uint32_t)path, (uint32_t)argv, (uint32_t)envp);
  if (res < 0) {
    errno = -res;
    res = -1;
  }
...
{
  int32_t res = _syscall2(SYSCALL_SYMLINK, (uint32_t)target, (uint32_t)linkpath);
  if (res < 0) {
    errno = -res;
    res = -1;
  }
...
{
  int32_t res = _syscall3(SYSCALL_READLINK, (uint32_t)path, (uint32_t)buf, bufsize);
  if (res < 0) {
    errno = -res;
    res = -1;
  }
...
{
  int32_t res = _syscall3(SYSCALL_LSEEK, fd, (uint32_t)offset, (uint32_t)whence);
  if (res < 0) {
    errno = -res;
    res = -1;
  }
